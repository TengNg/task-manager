...
describe('PUT /cards/:id/due-date/update', () => {
    let token;
    ...
    it('should return 404 if card is not found', async () => {
        ...
        const res = await request(app)
            .put(`/cards/${new mongoose.Types.ObjectId().toString()}/due-date/update`)
    ...
    it('should return 403 if user is not found', async () => {
        ...
        const res = await request(app)
            .put(`/cards/${mockCard._id}/due-date/update`)
            .set('Authorization', `Bearer ${token}`);
    ...
    it('should return 200 if card & user are found', async () => {
        ...
        expect(res.body).toEqual({ dueDate: "2024-12-31" });
    });
});

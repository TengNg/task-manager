...
export default function ListMenu({ list, open, setOpen, handleDelete, handleCopy }) {
    ...
    return (
        <div
            ...
            ref={containerRef}
            onBlur={closeOnBlur}
            className={`absolute top-0 left-0 outline-none z-10 bg-gray-100 border-gray-600 border-[2px] shadow-gray-600 box--style w-[225px] sm:w-[280px] p-3 ${theme.itemTheme == 'rounded' ? 'rounded-md shadow-[0_4px_0_0]' : 'shadow-[4px_6px_0_0]'}`}
        >

            <div className='border-b-[1px] border-b-black pb-2'>
                <div className='text-[9px] sm:text-[0.75rem]'>title: <span className='font-medium underline'>{list.title}</span></div>
                <div className='text-[9px] sm:text-[0.75rem]'>created at: {dateFormatter(list.createdAt)}</div>
            </div>

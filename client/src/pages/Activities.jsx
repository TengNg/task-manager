...
const Activities = () => {
    ...
    const handleRemoveInvitation = async (invitationId) => {
        try {
            const response = await axiosPrivate.delete(`/invitations/${invitationId}`, JSON.stringify({ id: invitationId }));
            console.log(response.data);
            setInvitations(prev => {
                            ...
                            {invitations.map((item, index) => {
                                ...
                                return <div
                                    ...
                                    className={`button--style--rounded border-black flex items-center p-3 py-4 rounded-lg hover:opacity-80 transition-all
                                                        ${status === "accepted" ? 'bg-blue-200 cursor-pointer' : status === "rejected" ? 'bg-red-200' : 'bg-gray-50'}`}
                                ...
                                >
...

                                    {
                                        status === 'pending'
                                            ...
                                            : <button
                                                onClick={() => handleRemoveInvitation(_id)}

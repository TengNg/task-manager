...
const Activities = () => {
    ...
    const acceptInvitationMutate = useMutation({
        ...
        onMutate: async (invitationId) => {
            ...
            const previousData = queryClient.getQueryData({ queryKey: ["invitations"] });
    ...
    const rejectInvitationMutate = useMutation({
        ...
        // },
        onError: (err) => {
            const errMsg = err.response?.data?.msg || "Failed to reject this invitation";
            alert(errMsg);
        },
    });

    const removeInvitationMutate = useMutation({
        ...
        // },
        onError: (err) => {
            const errMsg = err.response?.data?.msg || "Failed to remove this invitation";
            alert(errMsg);
        },
    });
    ...
    const acceptBoardRequestMutate = useMutation({
        mutationFn: ({
            id, boardId, requesterName
        }) => handleAcceptBoardRequest({ id, boardId, requesterName }),
        onMutate: async (variables) => {
            const { id: requestId } = variables;
            await queryClient.cancelQueries({ queryKey: ["boardRequests"] });
            const previousData = queryClient.getQueryData({ queryKey: ["boardRequests"] });
        ...
        // },
        onError: (err) => {
            const errMsg = err.response?.data?.msg || "Failed to remove this board-request";
            alert(errMsg);
        },
    });

    const rejectBoardRequestMutate = useMutation({
        mutationFn: ({
            id, boardId, requesterName
        ...
        onMutate: async (variables) => {
            ...
            queryClient.setQueryData(["boardRequests"], (old) => {
                return { 
                    ...
                    pages: [...old.pages].map(page => {
                        return page.map(item => {
                            return item._id === requestId ? { ...item, status: "rejected" } : item;
                        })
                    })
                }
            });
        ...
        // },
        onError: (err) => {
            const errMsg = err.response?.data?.msg || "Failed to reject this board-request";
            alert(errMsg);
        },
    });

    const removeBoardRequestMutate = useMutation({
        mutationFn: ({
            id, boardId, requesterName
            ...
            <section className="w-full h-full overflow-auto pb-8">
                ...
                <div className="flex flex-col gap-4">
                    <Invitations
                        ...
                        fetchNextPage={fetchNextInvitationPage}
                        hasNextPage={hasNextInvitationPage}
                        accept={acceptInvitationMutate}
                        reject={rejectInvitationMutate}
                        remove={removeInvitationMutate}
                    />
                    <JoinBoardRequests
                        ...
                        hasNextPage={hasNextBoardRequestPage}
                        isFetchingNextPage={isFetchingNextBoardRequestPage}
                        accept={acceptBoardRequestMutate}
                        reject={rejectBoardRequestMutate}
                        remove={removeBoardRequestMutate}
                    />
                </div>
            </section>
